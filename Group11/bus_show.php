...
<center>
	...
	$sql = "SELECT * FROM business WHERE SHIP='處理中' ORDER BY NUM ASC"; //在test資料表中選擇所有欄位，以ID為遞增排序
	...
	?>
    目前總共　<?php echo $total_records; ?>　筆訂單。
    <p>
    <form id="form" name="form" method="post" action="bus_search.php">請選擇欲查詢之條件：
    ...
    <p>
    <form id="form1" name="form1" method="post" action="bus_del.php">
    請輸入欲刪除之訂單編號： <input type="text"  size="15" name="id" id="textfield" />
    <input type="submit" name="button" value="刪除" onclick="if(confirm('確定要刪除此筆資料嗎？'))return true;else return false;" />
    </form>
    <p>
    ...
    <td width="300" align="left" valign="middle" bgcolor="#33CCFF"><?php echo $row[REMARK]; ?></td>
    <td align="center" valign="middle" bgcolor="#33CCFF"><a href="bus_add.php?num=<?php echo $row[NUM]; ?>&amp; name=<?php echo $row[NAME]; ?>&amp; phone=<?php echo $row[PHONE]; ?>&amp; area=<?php echo $row[AREA]; ?>&amp; adds=<?php echo $row[ADDS]; ?>&amp; date=<?php echo $row[DATE]; ?>&amp; item=<?php echo $row[ITEM]; ?>&amp; quan=<?php echo $row[QUAN]; ?>&amp; remark=<?php echo $row[REMARK]; ?>&amp">修改</a></td>
    </tr>
    <?php  }}  ?>
    </table>
    <p>
    ...
    $link=mysqli_connect("localhost","root","1234") or die ("無法開啟Mysql資料庫連結"); //建立mysql資料庫連
	mysqli_select_db($link, "project"); //選擇資料庫
	$sql = "SELECT * FROM business WHERE SHIP='已出貨' ORDER BY NUM ASC"; //在test資料表中選擇所有欄位，以ID為遞增排序
	mysqli_query($link, 'SET CHARACTER SET utf8');	 // 送出Big5編碼的MySQL指令 
	mysqli_query($link, "SET collation_connection = 'utf8_general_ci'");
	$result = mysqli_query($link,$sql); // 執行SQL查詢
	//$row = mysqli_fetch_assoc($result); //將陣列以欄位名索引
	...
	$total_records=mysqli_num_rows($result);  // 取得記錄數
	?>
    目前總共　<?php echo $total_records; ?>　筆訂單已出貨，<a href="bus_chart.php">查看銷售狀況</a>。<br>
    <table border="1" align="center" style="border:3px solid;padding:5px;" rules="all" cellpadding='5'>
    <tr>
    <td align="center" valign="middle"><div align="center">訂單編號</div></td>
    <td align="center" valign="middle"><div align="center">收件人姓名</div></td>
